...
It contains: classes to model a rectangle or an oval, the utility class containing methods used to modify shapes, the class that represent a snapshot, and the class that represent an album of snapshots. Relevant interfaces are also included. 

2. shapescontroller
This package's function is to convert the input file into an album. It reads in a txt file with commands including creation and modification of shapes, and taking snapshots. The output will be an album. 
It contains the controller for the model, and the controller's interface. 
